...
router.post('/', requireAuth, (req, res) => {
  ...
  const { amount, currency = 'USD', category = '', description = '', date } = req.body;
  if (!amount || !date) return res.status(400).json({ error: 'amount and date required' });
  const id = uuidv4();
  const created_at = new Date().toISOString();
  db.prepare('INSERT INTO expenses (id, company_id, user_id, amount, currency, category, description, date, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(id, req.user.company_id, req.user.id, amount, currency, category, description, date, 'pending', created_at);
